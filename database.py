...
def save_progress(user_id, topic_name, round_number, passed, score):
    if not is_db_connected(): return
    res = supabase.table('progress').select('*').eq('user_id', user_id).eq('topic_name', topic_name).eq('round_number', round_number).execute()
